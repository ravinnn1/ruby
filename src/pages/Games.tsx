  ...
  useEffect(() => {
    const spawn = () => {
      setBubbles(prev => [
        ...
        { id: nextId.current++, x: 5 + Math.random() * 85, y: 5 + Math.random() * 85, size: 36 + Math.random() * 36, color: BUBBLE_COLORS[Math.floor(Math.random() * BUBBLE_COLORS.length)], popped: false },
      ])
    }
    spawn()
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative rounded-3xl overflow-hidden" style={{ height: 260, background: 'linear-gradient(160deg, rgba(248,200,220,0.2), rgba(168,198,134,0.15))' }}>
        {bubbles.map(b => (
          <motion.button key={b.id} initial={{ scale: 0, opacity: 0 }} animate={b.popped ? { scale: 1.6, opacity: 0 } : { scale: 1, opacity: 1 }} transition={{ duration: b.popped ? 0.25 : 0.3, type: 'spring' }} onClick={() => !b.popped && pop(b.id)} className="absolute rounded-full border-2 border-white/40 cursor-pointer" style={{ left: `${b.x}%`, top: `${b.y}%`, width: b.size, height: b.size, background: `radial-gradient(circle at 35% 35%, white, ${b.color})`, boxShadow: `0 4px 16px ${b.color}60`, transform: 'translate(-50%, -50%)' }} aria-label="Pop bubble" />
        ))}
      </div>
    </div>
  )
